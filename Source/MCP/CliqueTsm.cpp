...
static thread_local int * Adj_List;
#define New_Name Node_Degree

static thread_local int FORMAT = 1, NB_NODE, NB_NODE_O, NB_EDGE, NB_EDGE_O, MAX_CLQ_SIZE, INIT_CLQ_SIZE, INIT_ORDERING, NB_BACK_CLIQUE, MATRIX_ROW_WIDTH, MATRIX_SIZE = 0, MAX_SUBGRAPH_SIZE, K_CORE_G = 0;

static thread_local long long MAX_CLQ_WEIGHT, CUR_CLQ_WEIGHT, INIT_CLQ_WEIGHT, OPT_CLQ_WEIGHT, MAX_ISET_WEIGHT, CUR_NODE_WEIGHT, UPPER_WEIGHT_BOUND;
static thread_local long long Max_Degree = 0;
static thread_local long long Node_Degree[MAX_NODE];
//static thread_local int Max_Weight = 0;
static thread_local long long Top_Weight[MAX_NODE];
static thread_local long long Node_Weight[MAX_NODE];
static thread_local char Node_Value[MAX_NODE];
...
static thread_local int Cursor_Stack_fill_pointer = 0;
static thread_local int Weight_Mod = 200;
static thread_local unsigned char * Adj_Matrix;

static thread_local int iSET_COUNT = 0;
static thread_local long long iSET_TOTAL_WEIGHT = 0;
static thread_local int *iSET_Size;
static thread_local long long *iSET_Weight;
static thread_local char *iSET_Tested;
static thread_local long long **iSET;
static thread_local int NEW_NODE_IDX = 0, MAX_OLD_NODE = 0, MAX_ISET_COUNT = 0;
static thread_local int RESERVED_LENGTH = 100;
static thread_local int LARGE_WEIGHT = FALSE;

#pragma warning(disable:4996)
/* the structures for maxsat reasoning*/

static thread_local struct iSET_State *IS;
static thread_local long long *iNode_TAIL;
...
static thread_local long long *REASON_STACK;
static thread_local int REASON_STACK_fill_pointer = 0;
static thread_local int *UNIT_STACK;
static thread_local int UNIT_STACK_fill_pointer = 0;
//static thread_local int *TOP_UNIT_STACK;
//static thread_local int TOP_UNIT_STACK_fill_pointer = 0;
static thread_local int *NEW_UNIT_STACK;
static thread_local int NEW_UNIT_STACK_fill_pointer = 0;
static thread_local int *FIXED_NODES_STACK;
static thread_local int FIXED_NODES_STACK_fill_pointer = 0;
static thread_local int *SATISFIED_iSETS_STACK;
static thread_local int SATISFIED_iSETS_STACK_fill_pointer = 0;
static thread_local int *TOPK_REDUCED_STACK;
static thread_local int TOPK_REDUCED_STACK_fill_pointer = 0;

static thread_local int Rollback_Point;
static thread_local int Branching_Point;
static thread_local int *Old_Name;
static thread_local int *Second_Name;
static thread_local int NB_CANDIDATE = 0, FIRST_INDEX;
static thread_local int START_MAXSAT_THD = 15;
static thread_local int BRANCHING_COUNT = 0;
static thread_local int CUR_MAX_NODE;

static thread_local int Last_Idx = 0;
static thread_local int cut_ver = 0, total_cut_ver = 0;
static thread_local int cut_inc = 0, total_cut_inc = 0;
static thread_local int cut_iset = 0, total_cut_iset = 0;
static thread_local int cut_satz = 0, total_cut_satz = 0;

static thread_local int LAST_IN;

/*statistical data*/
static thread_local long long N0_B, N0_A, N1_B, G1_B = 0;
static thread_local double D0_B, D0_A, D1_B = 0;
/*****************/
static thread_local int * Init_Adj_List;
...
static int re_number_adj(int node) {
	...
	long long *neibors, *saved_neibors = NULL, iset_weight, iset_second_weight, inc1 = 0, inc2 = 0;
...
static thread_local int NODE_IN_ISET;
...
static thread_local int NB_TOPK = 0;
...
static thread_local int MATRIX_REBUILDED = 0;
...
static thread_local int REBUILD_MATRIX = FALSE;
		...
		if(node==DELIMITER) {
			...
			if(Vertex_UB[CURSOR] <= MAX_CLQ_WEIGHT - CUR_CLQ_WEIGHT) {
				...
				if(Rollback_Point+MAX_SUBGRAPH_SIZE>=STACK_LENGTH) {
					printf("Logger::ERROR: the depth of search tree exceeds the limit of %d\n", STACK_LENGTH);
					exit(0);
				}
...
static thread_local int MATRIX_LIMITATION = 2048;
...
int tsmMain(int argc, char *argv[]) {
	int ret = 0;
	FORMAT = 1;
	INIT_ORDERING = 1;
	print_version();
...
int loadInput(NextEdge nextEdge, const Arr<Weight> &nodeWeights,
    ...
    int j, node = 1, l_node, r_node;
    long long weight, max_weight = 0;

    memset(Node_Degree, 0, MAX_NODE * sizeof(long long));

    NB_NODE = nodeWeights.size();
...
static void saveOutput(Clique &sln) {
    int i;
